...
public class LoverOfThree
{
    public static void Main()
    {
        ...
        int row = rows - 1;
        int col = 0;
        int[] directionRows = { -1, 1, 1, -1 };
        int[] directionCols = { 1, 1, -1, -1 };
        for (int i = 0; i < n; i++)
        {
            ...
            {
                int nextRow = row + directionRows[dir];
                int nextCol = col + directionCols[dir];
    ...
    private static int GetDirection(string dirName)
    {
        switch (dirName)
        {
            case "UR":
